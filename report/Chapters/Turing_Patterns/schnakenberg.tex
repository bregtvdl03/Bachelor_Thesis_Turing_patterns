The Schnakenberg equations, originally discussed in \cite{schnakenberg79}, form an example of a pattern forming reaction-diffusion system that is relevant for our models in \cref{ch:Bulk_surface_model,ch:Numerical_Analysis}. Their reaction functions are given by
%
\begin{equation}
    \label{eq:schnakenberg_reactions}
    f(u, v) = a - u + u^2 v \qquad \text{and} \qquad g(u,v) = b - u^2 v,
\end{equation}

\noindent
where $a$ and $b$ are positive real numbers representing constant production of both chemical species. In \cite{murray03}, a detailed analysis is given into the specific conditions for these reactions to form Turing patterns in one-dimensional domains. As a result, we get conditions for the parameters in the model that define a subset of the parameter space within which Turing instabilities take place, referred to as the \emph{Turing space}. For fixed $d$, this subset is given by the area between the two curves
%
\begin{align*}
    a &> \frac{1}{2} U \left(1 - U^2\right), & b &= \frac{1}{2} U \left(1 + U^2\right), \\
    a &< \frac{1}{2} U \left(1 - \frac{2U}{\sqrt{d}} - \frac{U^2}{d}\right), & b &= \frac{1}{2} U \left(1 + \frac{2U}{\sqrt{d}} + \frac{U^2}{d}\right),
\end{align*}

\noindent
parameterised by $U$, see \cref{fig:turingspace}. If $(a, b)$ lies between these two curves and $\gamma$ is high enough, a Turing instability will take place. You can see that the Turing space increases if larger values of $d$ are chosen. In \cref{fig:schnakenberg_frames_1-3,fig:schnakenberg_frames_4-6}, a simulation of this model is shown with parameters within the Turing space. In this simulation, the domain is $\Omega = (-8, 8)^2$ and the parameters are set to $d = 50$, $a = 0.1$, $b = 0.9$ and $\gamma = 12$. The initial conditions are set to
%
\begin{equation}\label{eq:schnakenberg_initial}
    u_0(x) = U + F(x) \qquad \text{and} \qquad v_0(x) = V + G(x),
\end{equation}

\noindent
where
%
\begin{equation}
    \label{eq:schnakenberg_steadystates}
    U := a + b \qquad \text{and} \qquad V := \frac{b}{(a + b)^2}
\end{equation}

\noindent
are the corresponding uniform steady states and $F$ and $G$ are random perturbations whose values are uniformly sampled from $(-0.1, 0.1)$. For more details on this perturbation of the uniform steady states, see \cref{ch:Numerical_Analysis}.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.5\linewidth]{img/Turing_space.png}
    \caption{The Turing space for \cref{eq:schnakenberg_reactions} plotted for $d = 20$, $d = 50$ and $d = 100$.}
    \label{fig:turingspace}
\end{figure}

\begin{figure}[htbp]
    \centering

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_1.png}
    \end{subfigure}

    \vspace{0.4cm}

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_2.png}
    \end{subfigure}

    \vspace{0.4cm}

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_3.png}
    \end{subfigure}

    \caption{A simulation of the Schnakenberg equations on $\Omega = (-8, 8)^2$ with $d = 50$, $a = 0.1$, $b = 0.9$ and $\gamma = 12$. The solution is plotted for times $t = 0$, $t = 5/\gamma$ and $t = 11/\gamma$.}
    \label{fig:schnakenberg_frames_1-3}
\end{figure}

\begin{figure}[htbp]
    \centering

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_4.png}
    \end{subfigure}

    \vspace{0.4cm}

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_5.png}
    \end{subfigure}

    \vspace{0.4cm}

    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/simulation_frames/schnakenberg_6.png}
    \end{subfigure}

    \caption{The same simulation of the Schnakenberg equations on $\Omega = (-8, 8)^2$ with $d = 50$, $a = 0.1$, $b = 0.9$ and $\gamma = 12$. The solution is now plotted for times $t = 14$, $t = 20/\gamma$ and $t = 80/\gamma$.}
    \label{fig:schnakenberg_frames_4-6}
\end{figure}
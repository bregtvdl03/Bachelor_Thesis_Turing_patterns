...
We derive a model that is similar to the ones used in \cite{iber2014}.The ligands can move freely through the extracellular space. This can be modelled by the standard diffusion (or heat) equation. To simplify the setting, we assume the individual receptors will be fixed in their position on the cell membrane.
...
Denote the diffusion coefficient by $D_v > 0$ and the outward unit normal vector on $\partial \Omega$ by $\nu$. Since the dynamics of the ligands include diffusion in $\Omega$ and reaction on $\Gamma$, they can be described by the PDE and boundary conditions given by
%
\begin{equation}
    \label{sys:PDEs}
    \begin{aligned}

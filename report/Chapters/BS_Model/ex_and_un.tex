...
%
    ...
    \noindent
    where $M = \esssup(u_0 + 2 u_{b, 0})$ and $T \geq \widetilde{T}$ is the original final time, for almost every $x \in \Gamma$ and $t \in [0, \beta]$.
...
%
    ...
    \noindent
    where we used the non-negativity of $v_-$ and $u_b$ in the last step. By \cref{thm:lionsmagenes}, we know that $v$ is continuous in time with respect to the $L^2$-norm. The only way for this to be achieved while also guaranteeing that the time derivative is non-positive, is by having
%
    \begin{equation*}

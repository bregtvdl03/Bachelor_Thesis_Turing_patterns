...
\noindent
where $\beta := \frac{\kon \delta_{u_b}}{\delta_{u_b} + \koff}$ and we make the second assumption, namely that $\alpha = 3\beta$. The final assumption made in \cite{iber2014} is that $\delta_v v \ll \beta u^2 v$, making the natural decay of the ligands negligible and recovering the classical Schnakenberg equations. However, as we saw in \cref{ch:Turing_patterns}, the Schnakenberg equations with additional decay of $v$ is also able to generate patterns, so we will not be making this assumption.
...
Additionally, we want a discretised function space that we use to construct approximate solutions to \cref{eq:fem_ex_poisson,eq:fem_ex_neumann_pois}, called the \emph{trial space}. To this end, for every node $x_i$, we define a basis function $\phi_i: \Omega_h \to \R$ such that $\phi_i(x_i) = 1$, $\phi_i(x_j) = 0$ for all $j \neq i$ and $\phi_i$ is linearly interpolated on each element. \todo{example} Now the function space we consider for our trial functions is the space of linear combinations of these basis functions,
...
Before deriving the linear system for \cref{eq:threespeciesmodel}, we should quickly illustrate how time discretisation ties into this approach. Consider, therefore, the heat equation
...
\noindent
To this end, we use the difference quotient to approximate the time derivative,
...
\begin{equation*}
    u_{n + 1}(x) \approx \tau \Delta u_{n + 1}(x) + u_n(x) + \tau f(n \tau, x), \qquad n = 0, \dots, N - 1.
\end{equation*}

...
\begin{equation}
        r_{n + 1} = r_{n} + \tau \gamma \left( a - r_{n} + r_{n}^{2}l_{n} \right) \quad \text{on } \partial C, n \in \mathbb{N},
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \int_{E} l_n \phi &= \int_{E} \left[ l_{n + 1} \phi - \tau d \Delta l_{n + 1} \phi \right] \\
...
\noindent
where in the last step we used that $\partial E$ is the disjoint union of $\partial \Omega$ and $\partial C$, since $\overline{C} \subseteq \Omega$, and the Neumann boundary conditions.

\begin{equation}
    \int_{\partial C} r_{n + 1} \phi = \int_{\partial C} \left[ r_{n} \phi + \tau \gamma \left( a - r_{n} + r_{n}^{2}l_{n} \right) \phi \right]
\end{equation}
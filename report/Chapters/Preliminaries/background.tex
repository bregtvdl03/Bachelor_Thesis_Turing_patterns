...
\begin{lemma}{Gronwall's inequality: differential form}{gronwall_dif}
...
This is known as the \emph{differential form} of Gronwall's inequality. We also have an \emph{integral form}, which is especially useful when dealing with non-differentiable functions. 

\begin{lemma}{Gronwall's inequality: integral form}{gronwall_int}
    Let $T > 0$ and let $\xi$ be a non-negative, $L^1$ function on $[0, T]$ which satisfies

    \begin{equation*}
        \xi(t) \leq C_1 \int_{0}^{t} \xi(s) \d s + C_2
    \end{equation*}

    \noindent
    for almost every $t \in [0, T]$ and constants $C_1, C_2 \geq 0$. Then

    \begin{equation*}
        \xi(t) \leq C_2 \left( 1 + C_1 t e^{C_1 t} \right)
    \end{equation*}

    for almost every $t \in [0, T]$.
\end{lemma}

Both of their proofs can be found in \cite{evans2010partial}.

...
gmsh.model.add("square_with_disks")
...
centers = list(itertools.product([-L/4, L/4], repeat=dim))
for (cx, cy) in centers:
    holes.append(gmsh.model.occ.addDisk(cx, cy, 0, half_cell, half_cell))

gmsh.model.occ.synchronize()

disk_boundaries = [s[1] for s in gmsh.model.getBoundary([(dim, h) for h in holes])]
...
disk_boundaries = [5, 6, 7, 8]

gmsh.model.mesh.field.add("Distance", DISTANCE_FIELD_TAG)
gmsh.model.mesh.field.setNumbers(DISTANCE_FIELD_TAG, "CurvesList", disk_boundaries)
gmsh.model.mesh.field.setNumber(DISTANCE_FIELD_TAG, "Sampling", SAMPLING)
...
gmsh.write("meshes/2d_disks.msh")
